...
/***************************************************************************
   ...
   SPECIAL REQUIREMENTS FOR THIS EXERCISE ONLY:
      ...
      PLEASE DO NOT SUBMIT THIS COMMENT OR THIS ORIGINAL FILE TO THE "ASSIGNMENT
      CHECKER" OR THE INSTRUCTOR.
 ...
 **************************************************************************/
 ...
 //
 // Leaving the following macro defined enables the binary tree code. Commenting
 // it out enables the hash table code.  Completely remove this macro and all
 // references to it in the program you produce.
 //
#define TREE
...
void PrintTable(const TABLE* hashTable)
{
    ...
    for (bin = hashTable->firstBin; bin < end; ++bin)  // visit bins
    {
        ...
        {
            PrintTree(node);
            //if (node != NULL)                                 // if child exists
            //{
              //  PrintTree(node->left);                         // traverse left
                //printf("%4zu  %s\n", node->count, node->left);
                //printf("%4zu  %s\n", node->count, node->right);
              //  PrintTree(node->right);                        // traverse right
            //}
        }
...
int main(int argc, char* argv[])
{
    ...
    // reached.  For each string read the BuildTree function will be called to
    // update the tree.
    //
